...
if($_POST)
{
    ...
    $mike_subject        = 'Quote request sent from Superior Carpet and Upholstery Care public site'; //Subject line for emails
  ...
  /*  if(!isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
	...
	$user_Rooms     = filter_var($_POST["userRooms"], FILTER_SANITIZE_STRING);
	$user_Size     = filter_var($_POST["userSize"], FILTER_SANITIZE_STRING);
    ...
    */
	...
	$message = "Message from " . $user_Name . " 
                        Phone: " . $user_Phone . " 
                        Email: " . $user_Email . "
						Number of Rooms: " . $user_Rooms . "
						Room Size: " . $user_Size . "
                        Message: " . $user_Message ;
        // send mail
    $sentMail = @mail($mike_to_email, $mike_subject, $message, $from);
	...
	Send Email to User
	*/
	$subject        = 'Superior Carpet and Upholstery Care - Quote'; //Subject line for emails
   ...
   if(!$mail->Send())
    {
        ...
        $output = json_encode(array('type'=>'message', 'text' => 'Thank you '.$user_Name .'! Your quote has been received.  We will get back to you with a response as soon as possible.'));
        die($output);
    }
}
?>